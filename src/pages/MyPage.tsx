...
import { UseUserReadBooks } from '../hooks/UseUserReadBook'

export default function MyPage() {
  // 임시로 하드코딩한 유저 데이터
  const navigate = useNavigate()
  const { data } = UseUserReadBooks()
    ...
    <>
      <div className="px-10 md:px-32 xl:px-60 pt-14 md:pt-28">
        ...
        <p className="text-sm md:text-xl lg:text-2xl">
          지금까지 <span className="text-[#EC6B53]">{data?.bookCount}권</span>의
          책을 읽고{' '}
          <span className="text-[#EC6B53]">{data?.reviewCount}개</span>의
          독후감을 작성했어요.
        </p>
      </div>
      <div className="px-10 md:px-16 xl:px-32 py-10 md:py-20">
        ...
        {data?.bookCount === 0 && data?.bookResponses.length === 0 ? (
          <NoBook />
        ) : (
          <BookList bookList={data?.bookResponses ?? []} />
        )}
      </div>
    </>
  )
